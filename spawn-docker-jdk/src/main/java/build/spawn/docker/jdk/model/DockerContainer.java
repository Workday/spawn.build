...
import build.spawn.docker.jdk.command.AttachContainer;
import build.spawn.docker.jdk.command.CopyFiles;
import build.spawn.docker.jdk.command.CreateExecution;
import build.spawn.docker.jdk.command.DeleteContainer;
import build.spawn.docker.jdk.command.FileInformation;
import build.spawn.docker.jdk.command.InspectContainer;
import build.spawn.docker.jdk.command.KillContainer;
import build.spawn.docker.jdk.command.PauseContainer;
import build.spawn.docker.jdk.command.StartExecution;
import build.spawn.docker.jdk.command.StopContainer;
import build.spawn.docker.jdk.command.UnpauseContainer;
        ...
        return new Executable() {
            ...
            public Execution execute() {
                ...
                final var id = createContext()
                    .inject(new CreateExecution(DockerContainer.this, terminalRequired, configuration))
                    .submit();

                // attempt to start the exec instance
                return createContext()
                    .inject(new StartExecution(DockerContainer.this, id, terminalRequired, configuration))
                    .submit();
            }
        };
    }
